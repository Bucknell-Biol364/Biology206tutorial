...
First, lets start by identifying a node within your tree. Identifying shared nodes will be useful later on when you want to find clades and other types of phyletic groups.
```{r Highlighting nodes}
ggtree(test_tree) + geom_tiplab() + geom_nodepoint(color="yellow", alpha=.9)
```
Great! But what if you only want to higlight certain nodes?
