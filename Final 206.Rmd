...
ggtree(test_tree) + geom_tiplab(hjust = -.2) + geom_nodepoint(color="yellow", alpha=.9, size= 4)
```
...
```{r}
plant_tree <- ggtree(plant_phylo) + geom_tiplab()
plant_tree
```
...
2. Where did woody stems evolve? Find this node and label it "Evolution of woody stems".
...
ggtree(plant_tree, aes(color=group, linetype=group)) + geom_tiplab(aes(subset=group==2)) + geom_tiplab()
...
4. Label the two monophyletic clades with the order they belong in.

5. Where did roots and/or tracheids evolve? Highlight that node.

6. Note all the places where vascular elements evolved and link them
```{r}
ggtree(plant_tree)+ geom_tiplab()+ geom_taxalink(')
```

ggtree(tree) + geom_tiplab() + geom_taxalink('A', 'E') + 
  geom_taxalink('F', 'K', color='red', arrow=grid::arrow(length=grid::unit(0.02, "npc")))

7. Do Porella cordeans, Blasia pusilla, Haplomitrium cooperi belong to a monophyletic (clade), polyphyletic or paraphyletic group? 
