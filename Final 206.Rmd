...
You can also mark just one node
```{r Highlighting one node}
geom_point2(aes(subset=(node == 8)), size=4, shape=23, fill="purple")
```
You can find the node you want to label by using the code chunk "mapping nodes"
...
ggtree(plant_tree)+ geom_tiplab() + xlim(-10, 30) + 
  ...
  geom_point2(aes(subset=(node == 28)), size=4, shape=23, fill="tan") + 
  geom_point2(aes(subset=(node == 31)), size=4, shape=23, fill="tan") +
  geom_point2(aes(subset=(node == 39)), size=4, shape=23, fill="tan")
```
ggtree(test_tree) + geom_tiplab(hjust = -.2) + geom_nodepoint(color="yellow", alpha=.9, size= 4) + xlim(0, 5)
...
There's also a plant character table in github too, if you think it would be useful for students
```{r}
plant_traits <- read.csv ("plant_character_table.csv", header=TRUE, sep=",")
```
