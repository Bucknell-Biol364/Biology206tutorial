---
title: "Final 206- answers included"
...
Now that you are familiar with the general layout of RStudio, let's begin with a tutorial!

SHOULD WE REMOVE THE TRAIT TABLE? I think we should keep it and tell them to refernece it if they want an explanation of the tree. It's also a way for them to learn how to read in an excel file. 

Reading in a table from a .csv (a type of excel file)
This is a useful RStudio skill. This trait table will help you understand the test tree you will be constructing below.
...
Label the pines, ferns, and liverworts on your tree. Are these groups monophyletic, paraphyletic or polypheletic? Highlight the nodes on your tree. How many traits are shared between the pines and liverworts? How many traits are shared between the liverworts and ferns? Based on what you know, are pines or ferns more closely related to liverworts?
```{r}
ggtree(plant_phylo) + 
  geom_tiplab()+
  geom_cladelabel(node=38, label="Pines", color="red", offset=8, align=TRUE) +     geom_cladelabel(node = 24, label = "Liverworts", color = "darkgreen", offset = 10, align = TRUE) + geom_cladelabel(node = 31, label = "Ferns", color = "blue", offset = 12, align = TRUE)  + geom_nodepoint(color="yellow", alpha=.9, size= 4) + xlim(-10, 30)
```


3. Label the two angiosperm clade.
```{r}
ggtree(plant_tree)+
  geom_tiplab() +
  xlim(-7, 30) +
...
4. Where did roots and/or tracheids evolve? Highlight that node.
```{r}
...
5. Note all the places where vascular elements evolved and highlight them
```{r}
ggtree(plant_tree)+ geom_tiplab() + xlim(-10, 30) 
```

7. Link the an angiosperm tree to a gymnosperm tree with green.
```{r}
ggtree(plant_tree)+ geom_tiplab() + xlim(-10, 30)+ geom_taxalink('Carya_ovata', 'Pinus_strobilus', color='green')
```

8. Label the monophyletic groups at the base of the tree with the broad category they fall into. 
```{r}
ggtree(plant_tree)+
  geom_tiplab() +
  xlim(-7, 30) +
  geom_cladelabel(, label= "algae", color="blue", offset= 12, align=TRUE)
```
in progress 
