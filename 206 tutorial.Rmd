...
```{r plant common names}
plants_commonname_newick <- "(red_algae, green_algae, (liverwort1, liverwor2, liverwort3),(sphagnum_moss, fern_moss), (shining_clubmoss, running_cedar_clubmoss), (((marginal_woodfern, intermediate_woodfern),hayscented_fern),(horsetail)), (ginkgo, (white_pine, pitch_pine)), (false_blue_indigo, nightshade,(black_walnut, shagbark_hickory)));"
```
```{r}
plants_cn <- read.tree(text=plants_commonname_newick)
```
```{r}
plot(plants_cn,no.margin=TRUE, edge.width=1)
```
```{r}
ggplot(plants_cn)+
  geom_tree()+
